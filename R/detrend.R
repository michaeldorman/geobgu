...
#' data(dem)
#' dem1 = detrend(dem)
#' result = c(dem, dem1["trend"], dem1["resid"], along = 3)
#' result = st_set_dimensions(result, 3, values = c("obs", "trend", "resid"))
#' plot(round(result), text_values = TRUE, col = terrain.colors(11))
...
detrend = function(x, parallel = 1) {
  ...
  if(parallel > 1) {
    ...
    dat$pred = predict(fit, dat, cluster = cl)
  ...
  trend = sf::st_as_sf(dat[, c("x", "y", "pred")], coords = c("x", "y"))
  trend = stars::st_rasterize(trend, x)
  resid = sf::st_as_sf(dat[, c("x", "y", "resid")], coords = c("x", "y"))
